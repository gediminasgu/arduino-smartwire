	...
	if (bufferLength > 7) 
	{
		...
		if (calculateCRC(buffer, bufferLength - 2) == crc) // if the calculated crc matches the recieved crc continue
		{
			...
			if (function == 3)
			{
				...
				if (startingAddress >= 10000) {
					startingAddress -= 10000;
					unsigned int maxData = startingAddress + no_of_registers;
					for (index = startingAddress * 2; index < maxData * 2; index++)
					{
						frame[address] = EEPROM.read(index);
						address++;
					}	
				...
				else if (startingAddress < holdingRegsSize) // check exception 2 ILLEGAL DATA ADDRESS
				{
					unsigned int maxData = startingAddress + no_of_registers;
					unsigned int temp;
			...
			else if (function == 16)
			{
				...
				if (frame[6] == (bufferLength - 9)) 
				{
					...
					if (startingAddress >= 10000) {
						...
						for (index = startingAddress * 2; index < maxData * 2; index++)
						{
							EEPROM.write(index, frame[address]);
							address++;
						}	
